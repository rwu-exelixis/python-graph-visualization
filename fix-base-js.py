...
if os.path.exists(js_path):
    string  = open(js_path, 'r').read()
else:
    die(f'File {js_path} does not exist. Please check and revise this script.')
    
d = re.findall(r'Gt\({À:.+?}\)', string)
## two occurences of such pattern in the original file. if so, make a copy
if len(d) > 0:
    print(f'Found {len(d)} locations with pattern: ' + '`Gt\({À:.+?}\)`' )
    shutil.copy(js_path, js_ori_path)
    # construct replacement string.
    r = re.sub(r'(.):', r'"\1":', d[0])
    ## replace string and save to disk.
    string_new = string.replace(d[0], r)
    with open(js_path, 'w') as fo:
        fo.write(string_new)
    print(f'Successfully modified file at: {js_path}')
    print(f'Back up file at: {js_ori_path}')
else:
...
will be changed by this script to:
